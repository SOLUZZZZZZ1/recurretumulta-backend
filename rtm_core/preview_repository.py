...
Las mutaciones se limitan a transiciones de estado. El contenido jurídico no
se edita después de crear la versión: cualquier cambio crea una versión nueva.
"""
...
LEGAL_PREVIEW_STORE_VERSION = "rtm_legal_preview_store_v1_0"
_FORBIDDEN_FAMILIES = {"", "generic", "unknown", "desconocido", "unresolved", "pendiente"}
...
def canonical_preview_json(preview: LegalPreview) -> str:
    return json.dumps(
        preview.model_dump(mode="json", exclude_none=False),
        ensure_ascii=False,
        sort_keys=True,
        separators=(",", ":"),
    )
...
def preview_digest(preview: LegalPreview) -> str:
    return hashlib.sha256(canonical_preview_json(preview).encode("utf-8")).hexdigest()


def validated_preview_copy(preview: LegalPreview, **updates: Any) -> LegalPreview:
    """Copia con validación completa; model_copy no revalida updates en Pydantic."""
...
def _row_to_record(row: Any) -> LegalPreviewRecord:
    ...
    payload = _json_payload(mapping["payload"])
    try:
        preview = LegalPreview.model_validate(payload)
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Previa Jurídica almacenada no válida: {exc}")

    expected_hash = preview_digest(preview)
    ...
    return LegalPreviewRecord(
        ...
        supersedes_id=(str(mapping["supersedes_id"]) if mapping.get("supersedes_id") else None),
        state_reason=mapping.get("state_reason"),
    )


_SELECT_PREVIEW = """
SELECT id, case_id, sequence, status, service, family, specialist,
       facts_version, family_resolution_version, payload, payload_sha256,
       created_by, created_at, approved_by, approved_at, frozen_by, frozen_at,
...
def _case_for_preview(conn, case_id: str) -> Mapping[str, Any]:
    ...
    if str(mapping["payment_status"]) != "paid":
        raise HTTPException(status_code=402, detail="El estudio debe estar pagado antes de crear la Previa Jurídica")
    ...
    if str(mapping["status"]) in _TERMINAL_CASE_STATUSES:
        raise HTTPException(status_code=409, detail="El expediente está en un estado final y no admite una nueva previa")
    return mapping


def get_preview(conn, case_id: str, preview_id: str, *, for_update: bool = False) -> LegalPreviewRecord:
...
def create_preview(
    ...
    *,
    ...
    if preview.status is not PreviewStatus.DRAFT:
        raise HTTPException(status_code=409, detail="Una nueva Previa Jurídica debe nacer en estado draft")
    ...
    if active:
        raise HTTPException(
            status_code=409,
            detail={
                "message": "Ya existe una Previa Jurídica activa; debe cerrarse o invalidarse antes de crear otra",
    ...
    if not supersedes_id and previous_latest and previous_latest.status in (
    ...
    if supersedes_id:
        ...
        if previous.status not in (PreviewStatus.CHANGES_REQUIRED, PreviewStatus.INVALIDATED):
            raise HTTPException(
                status_code=409,
                detail="La versión sustituida debe estar en changes_required o invalidated",
            )

    sequence_row = conn.execute(
        text("SELECT COALESCE(MAX(sequence), 0) + 1 FROM rtm_legal_previews WHERE case_id=:case_id"),
        {"case_id": case_id},
    ).fetchone()
    ...
    row = conn.execute(
        text(
            ...
            INSERT INTO rtm_legal_previews(
                case_id, sequence, status, service, family, specialist,
                facts_version, family_resolution_version, payload, payload_sha256,
                created_by, supersedes_id, created_at, updated_at
            ) VALUES (
                :case_id, :sequence, 'draft', :service, :family, :specialist,
                :facts_version, :family_resolution_version, CAST(:payload AS JSONB),
    ...
    conn.execute(text("UPDATE cases SET status='preview_draft', updated_at=NOW() WHERE id=:case_id"), {"case_id": case_id})
    _append_event(
        conn,
        case_id,
...
def _write_transition(
    ...
    *,
    ...
    fields: dict[str, Any] = {
        ...
        "invalidated_by": operator if preview.status is PreviewStatus.INVALIDATED else record.invalidated_by,
...
def submit_for_review(conn, case_id: str, preview_id: str, operator: str) -> LegalPreviewRecord:
    record = get_preview(conn, case_id, preview_id, for_update=True)
    if record.status is not PreviewStatus.DRAFT:
        raise HTTPException(status_code=409, detail="Solo una previa draft puede enviarse a revisión OPS")
    ...
    conn.execute(text("UPDATE cases SET status='preview_ops_review', updated_at=NOW() WHERE id=:case_id"), {"case_id": case_id})
    _append_event(conn, case_id, "rtm_legal_preview_submitted_for_review", {"preview_id": preview_id, "operator": operator})
    return updated
...
def request_changes(
    ...
    updated = _write_transition(conn, record, preview, operator=operator, state_reason=clean_reason)
    conn.execute(text("UPDATE cases SET status='preview_changes_required', updated_at=NOW() WHERE id=:case_id"), {"case_id": case_id})
    _append_event(conn, case_id, "rtm_legal_preview_changes_required", {"preview_id": preview_id, "operator": operator, "reason": clean_reason})
    return updated


def approve_preview(conn, case_id: str, preview_id: str, operator: str) -> LegalPreviewRecord:
    record = get_preview(conn, case_id, preview_id, for_update=True)
    ...
    updated = _write_transition(conn, record, preview, operator=operator)
    conn.execute(text("UPDATE cases SET status='preview_approved', updated_at=NOW() WHERE id=:case_id"), {"case_id": case_id})
    _append_event(conn, case_id, "rtm_legal_preview_approved", {"preview_id": preview_id, "operator": operator, "approved_at": now.isoformat()})
    return updated


def freeze_preview(conn, case_id: str, preview_id: str, operator: str) -> LegalPreviewRecord:
    record = get_preview(conn, case_id, preview_id, for_update=True)
    ...
    if blocking:
        raise HTTPException(
            status_code=409,
            detail={"message": "La previa contiene elementos bloqueantes", "blocking_items": blocking},
        )
    family = preview.family.strip().lower()
    if family in _FORBIDDEN_FAMILIES:
        raise HTTPException(status_code=409, detail="La familia jurídica no puede congelarse")
    for label, version in (
        ("facts_version", preview.facts_version),
        ("family_resolution_version", preview.family_resolution_version),
    ):
        low = version.strip().lower()
        if not low or any(token in low for token in ("pending", "unresolved", "pendiente")):
            raise HTTPException(status_code=409, detail=f"{label} no está consolidada")

    now = utcnow()
    frozen = validated_preview_copy(preview, status=PreviewStatus.FROZEN, frozen_at=now)
    updated = _write_transition(conn, record, frozen, operator=operator)
    conn.execute(text("UPDATE cases SET status='preview_frozen', updated_at=NOW() WHERE id=:case_id"), {"case_id": case_id})
    _append_event(
        conn,
        case_id,
...
def invalidate_preview(
    ...
    updated = _write_transition(conn, record, invalidated, operator=operator, state_reason=clean_reason)
    conn.execute(
        text(
            """
    ...
    case_row = conn.execute(text("SELECT COALESCE(status,'') FROM cases WHERE id=:case_id"), {"case_id": case_id}).fetchone()
    if case_row and str(case_row[0]) not in _TERMINAL_CASE_STATUSES:
        conn.execute(text("UPDATE cases SET status='manual_review', updated_at=NOW() WHERE id=:case_id"), {"case_id": case_id})
    _append_event(
        conn,
        case_id,
        "rtm_legal_preview_invalidated",
        {"preview_id": preview_id, "operator": operator, "reason": clean_reason, "invalidated_at": now.isoformat()},
    )
    return updated
