# -*- coding: utf-8 -*-
"""RTM — Velocity Decision Engine (v2, bucket)

Decide el modo jurídico en VELOCIDAD y añade un bucket de intensidad cuando procede.

Modes:
- inexistencia_infraccion
- error_tramo
- probatorio_puro
- falta_cuantificacion
- unknown

Bucket (solo cuando mode == probatorio_puro):
- leve  (exceso <= 10 km/h)
- medio (10 < exceso <= 30 km/h)
- grave (exceso > 30 km/h)

Este módulo NO genera el escrito. Devuelve un veredicto estructurado.
"""
...
def _expected_speed_sanction(limit: int, corrected: float) -> Dict[str, Any]:
    ...
    if lim is None:
        return {"fine": None, "points": None, "band": None, "table_limit": None, "corrected_int": v}
...
def _bucket_from_exceso(exceso_kmh: float) -> str:
    if exceso_kmh <= 10:
        return "leve"
    if exceso_kmh <= 30:
        return "medio"
...
def decide_modo_velocidad(core: Dict[str, Any], body: str = "", capture_mode: str = "UNKNOWN") -> Dict[str, Any]:
    ...
    if (measured is None or limit is None) and body:
        ...
        if m:
            if measured is None:
                measured = _to_int(m.group(1)); reasons.append("measured_from_body")
            if limit is None:
                limit = _to_int(m.group(2)); reasons.append("limit_from_body")
    ...
    if imposed_fine is not None and exp_f is not None and imposed_fine != exp_f:
        mismatch = True; reasons.append("fine_mismatch")
    if imposed_points is not None and exp_p is not None and imposed_points != exp_p:
        mismatch = True; reasons.append("points_mismatch")
    ...
    # probatorio puro + bucket
    bucket = _bucket_from_exceso(exceso)
    return {
