...
def _fix_solicito_format(body: str) -> str:
    if not body:
        return body
    # III. SOLICITO pegado con 1)
    body = re.sub(r"(III\.\s*SOLICITO)\s*(?=1[\)\.])", r"\1\n", body, flags=re.IGNORECASE)
    # punto 2 pegado
    body = re.sub(r"(\n1[)\.][^\n]*?)\s*2[)\.]", r"\1\n2)", body)
    # punto 3 pegado al final del 2
...
def _force_velocity_first_title(body: str) -> str:
    """
    Fuerza el título exacto de la ALEGACIÓN PRIMERA en velocidad.
    """
    if not body:
        return body
    target = "ALEGACIÓN PRIMERA — PRUEBA TÉCNICA, METROLOGÍA Y CADENA DE CUSTODIA (CINEMÓMETRO)"
    # reemplaza títulos típicos flojos
    patterns = [
        ...
        r"ALEGACIÓN\s+PRIMERA\s+—\s+PRESUNCIÓN\s+DE\s+INOCENCIA[\s\S]*?$",
    ]
    for p in patterns:
        body = re.sub(p, target, body, flags=re.IGNORECASE)
    # si existe ALEGACIÓN PRIMERA sin título fuerte, lo fuerza por seguridad
    body = re.sub(r"(ALEGACIÓN\s+PRIMERA\s+—\s+)([^\n]+)", r"\1" + target.split("—",1)[1].strip(), body, flags=re.IGNORECASE)
    return body
...
def _load_case_documents(case_id: str) -> List[Dict[str, Any]]:
    ...
    for i, r in enumerate(rows, start=1):
        ...
        docs.append(
            {"doc_index": i, "kind": kind, "bucket": bucket, "key": key, "mime": mime,
             "size_bytes": int(size_bytes or 0), "created_at": str(created_at), "text_excerpt": text_excerpt or ""}
        )
    return docs
...
def _detect_capture_mode(docs: List[Dict[str, Any]], extraction_core: Optional[Dict[str, Any]]) -> str:
    ...
    auto_signals = ["cámara","camara","fotograma","fotogramas","secuencia","captación automática","captacion automatica","sistema automático","sistema automatico","vídeo","video"]
    agent_signals = ["agente","policía","policia","guardia civil","presencial","in situ","observó","observo"]
...
def run_expediente_ai(case_id: str) -> Dict[str, Any]:
    ...
    # Heurística simple: si hay km/h => velocidad
    ...
    draft = _llm_json(
        PROMPT_DRAFT,
        {
            ...
            "attack_plan": {"infraction_type": infraction_type, "meta": {}},
            "facts_summary": "",
            "context_intensity": "critico" if infraction_type == "velocidad" else "normal",
    ...
    # Post-procesado determinista VELOCIDAD
    if isinstance(draft, dict) and infraction_type == "velocidad":
        ...
        cuerpo = _ensure_velocity_calc_paragraph(cuerpo, velocity_calc)  # primero cálculo
        cuerpo = _velocity_pro_enrich(cuerpo, velocity_calc)             # luego enrich
        cuerpo = _force_archivo_in_speed_body(cuerpo)
        cuerpo = _fix_solicito_format(cuerpo)
        draft["cuerpo"] = cuerpo

    # Auto-repair si falta mínimo
    if isinstance(draft, dict) and infraction_type == "velocidad":
        missing = _velocity_strict_missing(draft.get("cuerpo") or "")
        if missing:
            draft = _llm_json(
                PROMPT_DRAFT_REPAIR_VELOCIDAD,
                {"case_id": case_id, "missing": missing, "previous_draft": draft, "velocity_calc": velocity_calc},
            )
