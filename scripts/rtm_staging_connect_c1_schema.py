#!/usr/bin/env python3
"""Audita y aplica el esquema del Kernel RTM CONNECT C1 en staging.

``--apply`` exige ``STAGING_CONNECT_C1_SCHEMA_ONLY``. Solo ejecuta DDL aditivo,
idempotente y no destructivo. No publica rutas ni registra conectores.
"""
...
SCHEMA_VERSION = "rtm_staging_connect_c1_schema_v1_0"
APPLY_CONFIRMATION = "STAGING_CONNECT_C1_SCHEMA_ONLY"
...
def schema_snapshot(conn) -> dict[str, Any]:
    ...
    connect_tables, connect_missing = _snapshot_group(
        conn, CONNECT_C1_REQUIRED_COLUMNS
    )
    missing_indexes = sorted(CONNECT_C1_REQUIRED_INDEXES - _existing_indexes(conn))
    missing_triggers = sorted(CONNECT_C1_REQUIRED_TRIGGERS - _existing_triggers(conn))
    missing_constraints = sorted(
        CONNECT_C1_REQUIRED_CONSTRAINTS - _existing_constraints(conn)
    )
    return {
        "base": {
            "tables": base_tables,
        ...
        "connect_c1": {
            "tables": connect_tables,
            "missing_columns": connect_missing,
            "missing_indexes": missing_indexes,
            "missing_triggers": missing_triggers,
            "missing_constraints": missing_constraints,
            "ready": not (
                connect_missing or missing_indexes
                or missing_triggers or missing_constraints
            ),
        },
        "ready": not (
            base_missing or connect_missing or missing_indexes
            or missing_triggers or missing_constraints
        ),
    }
...
def apply_schema(conn) -> list[str]:
    ...
    applied: list[str] = []
    for name, statement in connect_c1_ddl():
    ...
    conn.execute(
        ...
        {
            "name": RTM_CONNECT_C1_SCHEMA_VERSION,
            "metadata": json.dumps(
                {
                    "source": SCHEMA_VERSION,
                    "architecture": "rtm_connect_architecture_v1_0",
                    "c0_manifest_required": True,
                    "tables": [
...
def main(argv: list[str] | None = None) -> int:
    ...
    report: dict[str, Any] = {
        "ok": False,
        "authority": "rtm_staging_connect_c1_schema",
        ...
        with engine.begin() as conn:
            ...
            report["blockers"] = list(report["blockers"]) + [
                ...
                for item in (
                    ...
                    + after["connect_c1"]["missing_columns"]
                    + after["connect_c1"]["missing_indexes"]
                    + after["connect_c1"]["missing_triggers"]
                    + after["connect_c1"]["missing_constraints"]
                )
            ]
