#!/usr/bin/env python3
"""Audita y aplica RTM CONNECT C3 manual_handoff solo en staging.

``--apply`` exige ``STAGING_CONNECT_C3_SCHEMA_ONLY``. El DDL es aditivo,
...
SCHEMA_VERSION = "rtm_staging_connect_c3_schema_v1_0"
APPLY_CONFIRMATION = "STAGING_CONNECT_C3_SCHEMA_ONLY"
...
BASE_REQUIRED_COLUMNS: dict[str, set[str]] = {
    ...
    "rtm_connect_connectors": {
        "id", "code", "version", "synthetic_only",
    },
    "rtm_connect_actions": {
...
def schema_snapshot(conn) -> dict[str, Any]:
    ...
    c3_tables, c3_missing = _snapshot_group(
        conn,
        CONNECT_C3_REQUIRED_COLUMNS,
    )
    missing_indexes = sorted(
        CONNECT_C3_REQUIRED_INDEXES - _existing_indexes(conn)
    )
    missing_triggers = sorted(
        CONNECT_C3_REQUIRED_TRIGGERS - _existing_triggers(conn)
    )
...
def apply_schema(conn) -> list[str]:
    ...
    applied: list[str] = []
    for name, statement in connect_c3_manual_ddl():
    ...
    conn.execute(
        ...
        {
            "name": RTM_CONNECT_C3_MANUAL_SCHEMA_VERSION,
            "metadata": json.dumps(
                {
...
def main(argv: list[str] | None = None) -> int:
    ...
    report: dict[str, Any] = {
        "ok": False,
        "authority": "rtm_staging_connect_c3_schema",
        ...
        with engine.begin() as conn:
            ...
            report["blockers"] = list(report["blockers"]) + [
                ...
                for item in (
                    ...
                    + after["connect_c3"]["missing_columns"]
                    + after["connect_c3"]["missing_indexes"]
                    + after["connect_c3"]["missing_triggers"]
                    + after["connect_c3"]["missing_constraints"]
                )
            ]
